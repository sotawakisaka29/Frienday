...
export function Inspector({
  ...
  const canvas = getCanvas(device, orientation);
  const defaultHeadlineFontSize = Math.round(
    Math.min(canvas.cW, canvas.cH) * (isFeatureGraphic ? 0.028 : 0.092),
  );
  const headlineFontSize = Math.round(slide.headlineFontSize ?? defaultHeadlineFontSize);
  const defaultLabelFontSize = Math.round(Math.min(canvas.cW, canvas.cH) * 0.028);
  const labelFontSize = Math.round(slide.labelFontSize ?? defaultLabelFontSize);
  const defaultLabelColor = theme.accent;
  const defaultHeadlineColor = slide.inverted ? theme.fgAlt : theme.fg;
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3 pt-1">
              <Input
                ...
                className="h-8 flex-1 cursor-pointer px-0"
              />
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                type="color"
                value={slide.labelColor ?? defaultLabelColor}
            ...
            <button
              ...
              onClick={() => onChange({ labelFontSize: undefined, labelColor: undefined })}
            >
              Reset Label ({defaultLabelFontSize}px · {defaultLabelColor})
            </button>
          </div>
        )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <div className="flex items-center gap-1">
              <Input
                aria-label="Headline font size in pixels"
                type="number"
                min={24}
                max={400}
                step={2}
                ...
                onChange={(e) => {
                  ...
                  if (Number.isFinite(value) && value >= 24) {
                    onChange({ headlineFontSize: value });
                  }
                }}
                className="h-8 w-20"
              />
          ...
          <button
            type="button"
            className="text-[10px] text-muted-foreground underline-offset-2 hover:underline"
            onClick={() => onChange({ headlineFontSize: undefined })}
          >
            Reset to responsive default ({defaultHeadlineFontSize}px)
          </button>
          <div className="flex items-center gap-2 pt-1">
            <Input
              aria-label="Headline color picker"
              type="color"
              value={slide.headlineColor ?? defaultHeadlineColor}
              onChange={(e) => onChange({ headlineColor: e.target.value })}
              className="h-9 w-12 cursor-pointer p-1"
            />
            <Input
              aria-label="Headline color code"
              value={slide.headlineColor ?? defaultHeadlineColor}
              onChange={(e) => onChange({ headlineColor: e.target.value })}
              className="h-9 font-mono text-xs"
            />
          ...
          <button
            type="button"
            className="text-[10px] text-muted-foreground underline-offset-2 hover:underline"
            onClick={() => onChange({ headlineColor: undefined })}
          >
            Reset Headline color ({defaultHeadlineColor})
          </button>
        </div>
